...
const initialRules: DetectionRule[] = [
  { id: "1", name: "Weapon Detection", category: "Object", target: "Firearms, Knives, Explosives", cameras: ["CAM-01", "CAM-02", "CAM-03"], confidenceThreshold: 0.6, alertSeverity: "HIGH", enabled: true, description: "Detect visible weapons including firearms, knives, and explosive devices" },
  { id: "2", name: "Intrusion Alert", category: "Behavior", target: "Unauthorized entry", cameras: ["CAM-01", "CAM-03"], confidenceThreshold: 0.7, alertSeverity: "HIGH", enabled: true, description: "Alert when person detected in restricted zone outside permitted hours" },
  { id: "3", name: "Loitering Detection", category: "Behavior", target: "Suspicious lingering", cameras: ["CAM-01", "CAM-02"], confidenceThreshold: 0.5, alertSeverity: "MEDIUM", enabled: true, description: "Detect individuals remaining in area for extended period" },
  { id: "4", name: "Perimeter Breach", category: "Zone", target: "Fence line crossing", cameras: ["CAM-03"], confidenceThreshold: 0.75, alertSeverity: "HIGH", enabled: true, description: "Detect movement crossing defined perimeter boundaries" },
  { id: "5", name: "Vehicle Tracking", category: "Object", target: "Unauthorized vehicles", cameras: ["CAM-02"], confidenceThreshold: 0.65, alertSeverity: "MEDIUM", enabled: false, description: "Track and identify vehicles not in authorized database" },
  { id: "6", name: "Face Match", category: "Facial", target: "Watchlist individuals", cameras: ["CAM-01", "CAM-02", "CAM-03"], confidenceThreshold: 0.8, alertSeverity: "HIGH", enabled: true, description: "Match detected faces against watchlist database" },
  { id: "7", name: "Crowd Density", category: "Behavior", target: "Unusual gathering", cameras: ["CAM-01", "CAM-02"], confidenceThreshold: 0.6, alertSeverity: "MEDIUM", enabled: true, description: "Alert when crowd density exceeds normal threshold" },
  { id: "8", name: "Abandoned Object", category: "Object", target: "Unattended bags/packages", cameras: ["CAM-01", "CAM-02", "CAM-03"], confidenceThreshold: 0.55, alertSeverity: "HIGH", enabled: true, description: "Detect objects left unattended for configurable duration" },
];
...
const DetectionRulesPage = () => {
  ...
  const toggleRule = async (id: string, currentStatus: boolean) => {
    try {
      const response = await fetch(`http://localhost:8001/rules/${id}`, {
        method: 'PATCH',
  ...
  const addRule = async () => {
    ...
    const payload = {
      ...newRule,
      cameras: "cam1" // Default to first camera for now
    };
                ...
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-4 text-xs text-muted-foreground">
                      ...
                      <span>📷 {rule.cameras.length > 0 ? rule.cameras.join(", ") : "All Cameras"}</span>
